...
#every variables has its own column
#every observations has its own row
#every values has its own cell
# R is to clean and organize data
...
```
# Only interested in lakeid and annnumber
```{r}
select(fish, "lakeid", "annnumber")
```
# Putting quotes to tell R that they are not objects

To add a range of columns use `start_col:end_col`.
```{r}
fish
```
# Anything between fishid and length
```{r}
...
# Names contain length
```{r}
select(fish, contains("length"))
```
# Names start with radii
...
# Names start with a and end with er
