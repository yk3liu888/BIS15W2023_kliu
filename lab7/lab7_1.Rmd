...
## Practice
1. Produce separate summaries of the mean and standard deviation for bill_length_mm, bill_depth_mm, and flipper_length_mm, # body_mass_g for each penguin species. Be sure to provide the number of samples.  
```{r}
penguins %>% 
  group_by(species) %>% 
  summarize(across(c(contains("mm"), body_mass_g), na.rm=T),
            n_samples=n())
```
```{r}
