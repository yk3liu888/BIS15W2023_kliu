...
```{r}
is.na(msleep) # not do this normally
```
...
```{r}
msleep %>% 
  summarise_all(~(sum(is.na(.)))) # ~ -> apply to all columns; . -> apply to all rows
```
...
```{r}
amniota %>% 
  summarize(number_nas = sum(is.na(female_body_mass_at_maturity_g)))
```
