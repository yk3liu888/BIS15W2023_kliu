...
deserts2 %>% 
  ...
  geom_line()+
  geom_point(shape=5)+ # shape 1 = circle, shape 2 = triangle
...
```{r}
grDevices::colors() # varieties of color
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(alpha = 0.4, color = "black", fill = "salmon", bins=40)+
  labs(title="Distribution of Homerange")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=after_stat(density)), alpha = 0.4, color = "black", fill = "salmon", bins=40)+
  geom_density(color="darkorchid4")+
  labs(title="Distribution of Homerange")
```
...
homerange <- homerange %>% 
             mutate(mass_category = case_when(log10.mass <= 1.75 ~ "small",
                                              ...
                                              log10.mass > 2.75 ~ "large")) # transformation for data that's mostly continuous
```
...
homerange %>% 
  ...
  labs(title="Observation by Range Category",
       x="Range ategory",
       y="Class")
```
