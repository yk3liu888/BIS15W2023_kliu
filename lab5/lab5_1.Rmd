...
```
```{r}
homerange %>% mutate_if(is.charater, as.factor)
```
...
# can't be AL and AR at the same time -> do not use &
```{r}
fish %>% 
...
mammals%>% 
  ...
  mutate(max_life_new = ifelse(newborn == -999.00, NA, max_life))%>% 
  arrange(family, Genus, species, max_life)
```
