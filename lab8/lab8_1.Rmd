...
```{r}
here() # put you in main directory # in the folder BIS15W2023_kliu
```
...
```{r}
heartrate %>% 
  pivot_longer(-patient, # except patient, rest of columns are wrong
               names_to = "drugs",# make a new column name
               values_to = "heartrate" # values move to
               )
```
...
Not all variables have their own column.
...
Not all variables with one column.
