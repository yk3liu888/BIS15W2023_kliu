...
+ `discrete` quantitative data that only contains integers ex. # of cards
+ `continuous` quantitative data that can take any numerical value ex. weight
+ `categorical` qualitative data that can take on a limited number of values ex. hair color
...
```{r}
ggplot(data=iris, mapping = aes(x=Sepal.Length, y=Sepal.Width))+geom_col()
```
...
```
# a lot of overlap -> overplotted
To add a regression (best of fit) line, we just add another layer.
```{r}
ggplot(data=homerange, mapping = aes(x=log10.mass, y=log10.hra))+geom_jitter()+geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(data=homerange, mapping = aes(x=log10.hra,
      y=log10.preymass))+geom_point(na.rm=T)+geom_smooth(method=lm, se=T)           #standard error bar-> shadow)
```
...
homerange %>% 
  group_by(class) %>% 
  summarize(mean_body_wt=mean(log10.mass)) %>% 
  ggplot(aes(y=class, x=mean_body_wt))+
  geom_col()
```
...
```{r}
homerange %>% 
  filter(class == "mammalia") %>% #count(trophic.guild)
  ggplot(aes(x=trophic.guild)) + #ggplot(aes(x=trophic.guild, y=n)) +
  geom_bar() #geom_col()
```
